...
function App() {
    ...
    map.on('load', async () => {
      ...
      map.addLayer({
        ...
        paint: {
          ...
          'fill-opacity': ['case', ['boolean', ['get', 'visited'], false], 0.45, 0.0],
        },
      });
      map.addLayer({
        id: 'country-outline',
      ...
      map.addLayer({
        ...
        filter: ['!', ['has', 'point_count']],
        paint: {
          'circle-color': ['coalesce', ['get', 'visit_color'], '#f97316'],
          'circle-radius': 6,
  ...
  useEffect(() => {
    ...
    const placeLookup = new Map(places.country.map((place) => [place.id, place]));
