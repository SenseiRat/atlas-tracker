...
def _compute_source_digests() -> dict[str, str]:
    ...
    for source_key, config in SOURCE_DATASET_DEFINITIONS.items():
        path_digests = [
            f"{path.name}:{_path_digest(path)}" for path in _source_paths(config) if path.exists()
        ]
        if path_digests:
...
def _load_source_payloads(source_keys: set[str] | None = None) -> dict[str, Any]:
...
def _collect_country_rows(
    ...
    for feature in payload.get("features", []):
        ...
        country_code = str(
            props.get("ADM0_A3") or props.get("ISO_A3") or props.get("NAME") or ""
        ).upper()
        ...
        source_state[place_id] = (
            "countries",
            hashlib.sha256(json.dumps(data, sort_keys=True).encode("utf-8")).hexdigest(),
        )
...
def _collect_state_rows(
    ...
    source_state: dict[str, tuple[str, str]] = {}
    iso2_to_iso3: dict[str, str] = context.setdefault("iso2_to_iso3", {})
    state_rows: dict[tuple[str, str], tuple[str, str, str, float | None, float | None, str]] = {}
    ...
    for state_row in state_rows.values():
        ...
        source_state[state_row[0]] = (
            "state_regions",
            hashlib.sha256(state_row[6].encode("utf-8")).hexdigest(),
        )
    return rows, source_state


def _collect_city_rows(
    payload: Any, context: dict[str, Any]
) -> tuple[list[tuple], dict[str, tuple[str, str]]]:
    rows: list[tuple] = []
    source_state: dict[str, tuple[str, str]] = {}
    ...
    for city in payload:
        ...
        country_code = normalize_country_code(
            city.get("country_code") or city.get("iso_country"), iso2_to_iso3
        )
        ...
        source_state[place_id] = (
            "cities",
            hashlib.sha256(json.dumps(city_payload, sort_keys=True).encode("utf-8")).hexdigest(),
        )
    return rows, source_state


def _collect_airport_rows(
    payload: Any, context: dict[str, Any]
) -> tuple[list[tuple], dict[str, tuple[str, str]]]:
    rows: list[tuple] = []
    source_state: dict[str, tuple[str, str]] = {}
    ...
    for airport in payload:
        ...
        country_code = normalize_country_code(
            airport.get("country_code") or airport.get("iso_country"), iso2_to_iso3
        )
        ...
        airport_id = (
            airport.get("id")
            or airport.get("ident")
            or airport.get("icao_code")
            or airport.get("iata_code")
            or airport.get("gps_code")
            or airport.get("local_code")
        )
        ...
        source_state[place_id] = (
            "airports",
            hashlib.sha256(json.dumps(airport_payload, sort_keys=True).encode("utf-8")).hexdigest(),
        )
    return rows, source_state


def _collect_site_rows(
    payload: Any, context: dict[str, Any]
) -> tuple[list[tuple], dict[str, tuple[str, str]]]:
    rows: list[tuple] = []
    source_state: dict[str, tuple[str, str]] = {}
    ...
    if (
        not payload_groups
        or not isinstance(payload_groups, list)
        or "payload" not in payload_groups[0]
    ):
    ...
    for payload_group in payload_groups:
        ...
        if filename == "festivals.json":
            ...
            for festival in source_entries:
                ...
                alternate_names = [
                    item
                    for item in (
                        sanitize_markup_text(value)
                        for value in (festival.get("alternate_names") or [])
                    )
                    if item
                ]
                country_or_countries = [
                    ...
                    for item in (
                        sanitize_markup_text(value)
                        for value in (festival.get("country_or_countries") or [])
                    )
                    if item
                ]
                resolved_country_codes = [
                    code
                    for code in (
                        infer_country_code_from_text(item, country_name_to_iso3)
                        for item in country_or_countries
                    )
                    if code
                ]
                ...
                tags = [
                    item
                    for item in (
                        sanitize_markup_text(value) for value in (festival.get("tags") or [])
                    )
                    if item
                ]
                site_id = (
                ...
                source_state[place_id] = (
                    "sites",
                    hashlib.sha256(
                        json.dumps(site_payload, sort_keys=True).encode("utf-8")
                    ).hexdigest(),
                )
            continue
        if filename == "michelin_restaurants.json":
            restaurant_entries = (
                source_payload.get("restaurants")
                if isinstance(source_payload, dict)
                else source_payload
            )
            ...
            for group_key, variants in grouped_entries.items():
                base_variant = next(
                    (
                        ...
                        and not str(sanitize_markup_text(item.get("name")) or "").startswith(
                            "Reserve a table "
                        )
                    ),
                    None,
                ...
                summary_variant = next(
                    (
                        item
                        for item in variants
                        if str(sanitize_markup_text(item.get("name")) or "").startswith(
                            "Reserve a table "
                        )
                    ),
                    None,
                ...
                country_code = infer_country_code_from_text(
                    country_label or location_text, country_name_to_iso3
                )
                price = (
                    sanitize_markup_text(representative.get("price")) or summary_details["price"]
                )
                cuisine = (
                    sanitize_markup_text(representative.get("cuisine"))
                    or summary_details["cuisine"]
                )
                distinction = (
                    sanitize_markup_text(representative.get("distinction"))
                ...
                site_payload = {
                    ...
                    "countryOrCountries": [country_label]
                    if country_label
                    else ([country_code] if country_code else []),
                    "region": None,
                    ...
                    "latitude": None,
                    "longitude": None,
                    "summary": ", ".join(item for item in [location_text, cuisine, price] if item)
                    or distinction,
                    "tags": tags,
        ...
        for site in source_payload:
            if filename == "whc001.json":
                name = (
                    sanitize_markup_text(site.get("name_en") or site.get("name")) or "Unknown site"
                )
                coordinates = site.get("coordinates") or {}
                raw_iso_codes = str(site.get("iso_codes") or "").strip()
                country_codes = [
                    code.strip().upper() for code in raw_iso_codes.split(",") if code.strip()
                ]
                country_code = normalize_country_code(
                    country_codes[0] if country_codes else None, iso2_to_iso3
                )
                ...
                if site_id is None:
                    site_id = (
                        f"unesco-{slugify(name)}-{country_code or 'xx'}-{lat or 'na'}-{lon or 'na'}"
                    )
                states_names = [
                    item
                    for item in (
                        sanitize_markup_text(value) for value in (site.get("states_names") or [])
                    )
                    if item
                ]
                ...
                site_payload = {
                    ...
                    "metadata": {
                        ...
                        "date_inscribed": site.get("date_inscribed"),
                        "danger": str(site.get("danger") or "").strip().lower() == "true",
                        "transboundary": str(site.get("transboundary") or "").strip().lower()
                        == "true",
                        "main_image_url": site.get("main_image_url"),
                    },
                }
                ...
                country_code = normalize_country_code(
                    site.get("country_code") or site.get("iso_country"), iso2_to_iso3
                )
...
def _collect_places_from_sources(
    ...
    for source_key in SOURCE_DATASET_DEFINITIONS:
        if source_key not in payloads:
            continue
        collector_rows, collector_state = SOURCE_COLLECTORS[source_key](
            payloads[source_key], context
        )
...
def _place_is_referenced(conn: DBConnection, place_id: str) -> bool:
    visit_ref = conn.execute(
        "SELECT 1 FROM visits WHERE place_id = ? LIMIT 1", (place_id,)
    ).fetchone()
    if visit_ref:
        return True
...
def sync_places_from_data_sources(
    conn: DBConnection, source_keys: set[str] | None = None
) -> dict[str, Any]:
...
def sync_data_sources_if_needed(*, force: bool = False, reason: str = "manual") -> dict[str, Any]:
    ...
    auto_sync_keys = {
        source_key
        for source_key, config in SOURCE_DATASET_DEFINITIONS.items()
        if bool(config.get("auto_sync"))
    }
    tracked_digests = {
        source_key: digests[source_key] for source_key in auto_sync_keys if source_key in digests
    }
    with get_db() as conn:
        ...
        if (
            not force
            and tracked_digests == previous_digests
            and previous_schema_version == DATA_SYNC_SCHEMA_VERSION
        ):
            return {
                "changed": False,
                "reason": reason,
